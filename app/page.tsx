        ...
        <section className="relative mt-2.5 overflow-hidden rounded-[20px] border border-[#d7b56d]/15 shadow-[0_18px_55px_rgba(0,0,0,.38)] sm:mt-4 sm:rounded-[28px]">
          ...
          <div className="relative min-h-[220px] px-4 py-6 sm:min-h-[300px] sm:px-8 sm:py-9 md:min-h-[330px] md:px-10">
            <div className="max-w-[510px]">
              <div className="inline-flex items-center gap-1.5 rounded-full border border-[#f5d88e]/20 bg-black/20 px-2.5 py-1 text-[8px] font-semibold uppercase tracking-[.20em] text-[#f5d88e] backdrop-blur-md sm:px-3 sm:py-1.5 sm:text-[9px]"><Sparkles className="h-3 w-3" /> Sana özel fal köşen</div>
              <h1 className="mt-3 font-serif text-[27px] font-bold leading-none tracking-[-.035em] text-white sm:mt-4 sm:text-[38px] md:text-[44px]">Merhaba {firstName} <span className="text-[#f5d78e]">✦</span></h1>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 lg:grid-cols-4">
            {discoveries.map((item) => (
              <Link key={item.title} href={item.href} className="group overflow-hidden rounded-[15px] border border-white/[.09] bg-[#111022] shadow-[0_8px_24px_rgba(0,0,0,.22)] transition duration-300 hover:-translate-y-0.5 hover:border-[#d5a6ff]/35 sm:rounded-[18px]">
                <div className="relative aspect-[1.42] overflow-hidden"><Image src={item.image} alt={item.title} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" className="object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#111022] via-transparent to-transparent opacity-90" /></div>
                <div className="relative -mt-5 px-2.5 pb-2.5 sm:-mt-7 sm:px-3.5 sm:pb-3.5"><h3 className="font-serif text-[13px] font-bold leading-tight text-white sm:text-lg">{item.title}</h3><p className="mt-0.5 line-clamp-2 text-[9px] leading-3.5 text-[#bcb3c9] sm:mt-1 sm:text-xs sm:leading-5">{item.subtitle}</p></div>
              </Link>
            ))}
          </div>
        </section>
