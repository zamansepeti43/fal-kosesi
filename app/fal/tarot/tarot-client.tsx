...
const spreads: Spread[] = [
  { id: "single", label: "Tek Kart", short: "Bugünün mesajı", description: "Aklındaki konu için tek ve net bir sembolik mesaj.", cards: 1, icon: Sparkles },
  { id: "three", label: "3 Kart", short: "Geçmiş • Şimdi • Gelecek", description: "Bir konunun akışını üç farklı açıdan gör.", cards: 3, icon: Wand2 },
  { id: "love", label: "Aşk Açılımı", short: "Kalbinin haritası", description: "Sen, karşı taraf ve aranızdaki bağın enerjisi.", cards: 3, icon: Heart },
  { id: "career", label: "Kariyer", short: "Yol • Engel • Fırsat", description: "İş, hedef ve ilerleme konusunda sembolik rehberlik.", cards: 3, icon: BriefcaseBusiness },
  { id: "money", label: "Para & Kısmet", short: "Enerji • Engel • Akış", description: "Maddi konularda dikkat edilmesi gereken temaları keşfet.", cards: 3, icon: CircleDollarSign },
];
...
function CardBack() {
  return <img src={TAROT_BACK} alt="Tarot kartının arka yüzü" className="h-full w-full object-cover" draggable={false} />;
}
...
function getPositionLabels(spread: SpreadId) {
  if (spread === "love") return ["Senin enerjin", "Karşı taraf", "Aranızdaki bağ"];
  if (spread === "career") return ["Mevcut yol", "Önündeki engel", "Fırsat / yön"];
  if (spread === "money") return ["Maddi enerji", "Dikkat edilmesi gereken", "Kısmetin akışı"];
  return ["Geçmiş", "Şimdi", "Gelecek"];
}

function getContextText(card: TarotCard, spread: SpreadId, position: number) {
  if (spread === "love") return card.love;
  if (spread === "career") return card.career;
  if (spread === "money") return `${card.meaning} Para ve kısmet tarafında özellikle ${card.keywords[0]} teması öne çıkıyor.`;
  if (spread === "three") {
...
export default function TarotClient() {
  ...
  const completed = selectedCards.length >= spread.cards && !revealing && creditStatus === "ok";
  ...
  const chooseCard = (card: TarotCard) => {
    if (revealing || selectedCards.length >= spread.cards || selectedCards.some((item) => item.id === card.id)) return;
  ...
  const availableCards = deck.filter((card) => !selectedCards.some((selected) => selected.id === card.id));
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top,rgba(124,58,237,.24),transparent_32%),radial-gradient(circle_at_80%_25%,rgba(245,158,11,.08),transparent_28%),linear-gradient(180deg,#080817,#05050d)] px-3 pb-10 pt-4 text-white sm:px-5 sm:pt-7">
      <div className="mx-auto max-w-5xl">
        ...
        {completed && <section className="mt-4 rounded-[28px] border border-amber-200/20 bg-[radial-gradient(circle_at_50%_0%,rgba(245,158,11,.12),transparent_38%),rgba(18,13,31,.97)] p-5 shadow-2xl sm:p-7"><div className="text-center"><p className="text-[9px] font-bold uppercase tracking-[.3em] text-amber-200">{name}, açılımın</p><h2 className="mt-1 font-serif text-3xl font-bold text-amber-100">Kartların sana ne söylüyor?</h2><p className="mx-auto mt-2 max-w-2xl text-xs leading-6 text-slate-400">Kartları tek tek değil, birbirleriyle kurdukları hikâye üzerinden değerlendir. Aynı tema tekrar ediyorsa özellikle dikkat çekiyor olabilir.</p></div><div className="mt-6 space-y-4">{selectedCards.map((card, index) => <article key={card.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[.025]"><div className="flex items-center gap-3 border-b border-white/5 p-3"><div className="w-[54px] overflow-hidden rounded-lg border border-amber-200/20"><CardFront card={card} /></div><div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-[.18em] text-amber-200">{positions[index]}</p><h3 className="mt-0.5 font-serif text-xl font-bold">{card.name}</h3><p className="text-[9px] text-slate-500">{card.keywords.join(" • ")}</p></div></div><div className="p-4"><p className="text-sm leading-7 text-slate-200">{getContextText(card, spreadId, index)}</p>{question.trim() && <p className="mt-3 rounded-xl border border-violet-300/10 bg-violet-400/[.05] p-3 text-xs leading-6 text-slate-300"><strong className="text-violet-200">Soruna göre:</strong> “{question.trim()}” konusunda bu kartın {card.keywords[0]} teması öne çıkıyor. Bunu kesin bir gelecek vaadi olarak değil, kararlarını düşünürken kullanabileceğin sembolik bir ayna olarak değerlendir.</p>}</div></article>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-3"><article className="rounded-2xl border border-rose-300/10 bg-rose-400/[.045] p-4"><div className="flex items-center gap-2 text-sm font-bold text-rose-200"><Heart className="h-4 w-4" /> Aşk</div><p className="mt-2 text-xs leading-6 text-slate-300">{selectedCards[0]?.love}</p></article><article className="rounded-2xl border border-sky-300/10 bg-sky-400/[.045] p-4"><div className="flex items-center gap-2 text-sm font-bold text-sky-200"><BriefcaseBusiness className="h-4 w-4" /> İş & Kariyer</div><p className="mt-2 text-xs leading-6 text-slate-300">{selectedCards[1]?.career || selectedCards[0]?.career}</p></article><article className="rounded-2xl border border-amber-300/10 bg-amber-400/[.045] p-4"><div className="flex items-center gap-2 text-sm font-bold text-amber-200"><CircleDollarSign className="h-4 w-4" /> Para & Kısmet</div><p className="mt-2 text-xs leading-6 text-slate-300">{selectedCards[selectedCards.length - 1]?.meaning}</p></article></div><div className="mt-5 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => resetReading()} className="flex items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[.04] px-5 py-3 text-sm font-semibold"><RotateCcw className="h-4 w-4" /> Yeni Açılım</button><Link href="/fal/premium" className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-amber-300 to-yellow-200 px-5 py-3 text-sm font-black text-slate-950"><Sparkles className="h-4 w-4" /> Derinlemesine Premium Yorum <ChevronRight className="h-4 w-4" /></Link></div></section>}

        {!completed && !activeCard && <section className="mt-4 rounded-2xl border border-violet-300/10 bg-white/[.025] p-4 text-center text-xs leading-6 text-slate-400"><Wand2 className="mx-auto mb-2 h-5 w-5 text-violet-300" />Kartları acele etmeden kaydır. İlk dikkatini çeken kartı seç; {spread.cards > 1 ? `açılım tamamlanana kadar ${spread.cards} kartı sırayla seç.` : "tek kartın mesajını aç."}</section>}
      </div>
    </main>
  );
}
