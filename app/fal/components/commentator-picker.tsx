...
function characterAvatar(item: Commentator, slot: number) {
  if (item.id.startsWith("tarot-") && TAROT_AVATAR_BY_ID[item.id]) return `/fortune/avatars/${TAROT_AVATAR_BY_ID[item.id]}?v=10`;
  return `/api/fal/avatar?id=${encodeURIComponent(item.id)}&slot=${slot}&v=3`;
}
