...
function characterAvatar(item: Commentator, slot: number) {
  ...
  // Explicit slot guarantees that the five visible non-Tarot characters never
  // collapse onto the same renderer avatar because of hash collisions.
  return `/api/fal/avatar?id=${encodeURIComponent(item.id)}&slot=${slot}`;
}
...
function characterFallback(item: Commentator) {
  ...
  // Non-Tarot portraits must never fall back to Tarot artwork. The avatar route
  // is same-origin and already proxies the curated 3D render, so on a transient
  // failure we simply leave the card artwork hidden rather than polluting Tarot.
  return "";
}
