  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/15 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-slate-100">AI falcını seç</p>
          <p className="mt-1 text-[11px] text-slate-500">Bunlar gerçek kişi değildir. Her sanal karakterin farklı uzmanlığı, yorum tonu, süresi ve kredi fiyatı vardır.</p>
        </div>
        {items.length > 3 && <button type="button" onClick={() => setShowAll((value) => !value)} className="text-xs text-violet-200">{showAll ? "3 göster" : "5 falcı"}</button>}
      </div>
      ...
      <div className="mt-3 grid gap-2">
        <button type="button" onClick={() => onSelect(null)} className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${selectedId === null ? "border-amber-300/50 bg-amber-300/[.08]" : "border-white/8 bg-white/[.02]"}`}>
          ...
          <span className="min-w-0 flex-1"><span className="block text-sm font-semibold">Otomatik seçim</span><span className="block text-[10px] text-slate-500">Sistem fiyat, hız ve uygunluğa göre seçer</span></span>
        </button>
        {visible.map((item) => {
          const isSelected = selectedId === item.id;
          return (
            <button key={item.id} type="button" onClick={() => onSelect(item)} className={`rounded-xl border p-3 text-left transition ${isSelected ? "border-violet-400/60 bg-violet-500/[.10]" : "border-white/8 bg-white/[.02]"}`}>
              <div className="flex items-start gap-3">
                <img src={item.avatarUrl} alt={`${item.name} AI karakter`} loading="lazy" className="h-14 w-14 shrink-0 rounded-xl bg-[#17152c] object-cover" />
                <span className="min-w-0 flex-1">
                  <span className="flex flex-wrap items-center gap-2"><strong className="text-sm text-slate-100">{item.name}</strong><span className="rounded-full bg-violet-400/10 px-1.5 py-0.5 text-[8px] font-bold text-violet-200">AI KARAKTER</span></span>
                ...
                <span className="shrink-0 text-right"><strong className="block text-sm text-amber-200">{item.priceCredits} kredi</strong><span className="mt-1 inline-flex items-center gap-1 text-[9px] text-slate-500"><Mic2 className="h-3 w-3" />+{item.voiceCredits}</span></span>
              </div>
              ...
              <span className="ml-2 text-[9px] text-slate-500"><Heart className="mr-1 inline h-3 w-3" />Favoriye ekle</span>
            </button>
          );
        })}
      </div>
