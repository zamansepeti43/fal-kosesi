...
function spriteStyle(slot: number): CSSProperties {
  return {
    backgroundImage: `url(${NON_TAROT_AVATAR_SPRITE})`,
    backgroundSize: "500% 100%",
    backgroundPosition: `${slot * 25}% center`,
  };
}
  ...
  return <div className="w-full px-0 py-1">
    ...
    <div className="-mx-1 grid grid-cols-2 items-start gap-4 sm:mx-0 sm:gap-5">
      {visible.map((item, index) => {
        ...
        const isTarot = item.id.startsWith("tarot-");
        return <button key={item.id} type="button" onClick={() => onSelect(item)} aria-pressed={isSelected} className={`group relative ${centeredLast} rounded-[24px] border p-2.5 text-left transition duration-300 sm:p-3 ${isSelected ? "-translate-y-0.5 border-amber-300/70 bg-amber-300/[.08] shadow-[0_0_30px_rgba(251,191,36,.12)]" : "border-white/8 bg-white/[.025] hover:-translate-y-1 hover:border-amber-200/30 hover:bg-white/[.045]"}`}>
          <div className="relative aspect-[1.15/1] w-full overflow-hidden rounded-[20px] border border-white/10 bg-[#100d18] shadow-inner" role={isTarot ? undefined : "img"} aria-label={isTarot ? undefined : `${item.name} sanal karakteri`}>
            {isTarot ? <img src={characterAvatar(item)} alt={`${item.name} sanal karakteri`} loading="eager" decoding="async" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.onerror = null; const fallback = characterFallback(item); if (fallback) event.currentTarget.src = fallback; else event.currentTarget.style.visibility = "hidden"; }} className="absolute inset-0 block h-full w-full object-cover object-center" /> : <div aria-hidden="true" className="absolute inset-0 h-full w-full bg-center bg-no-repeat" style={spriteStyle(index)} />}
