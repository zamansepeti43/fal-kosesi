...
export default function PremiumFortune({ kind, title, eyebrow, description, placeholder, accent = "violet" }: Props) {
  ...
  const [focus, setFocus] = useState(focusByKind[kind] ?? "genel");
  ...
  const runReading = async () => {
    ...
    try {
      ...
      if (status === 402) setError("Yeterli kredin yok. Devam etmek için kredi yükleyebilir veya Premium hizmete geçebilirsin.");
