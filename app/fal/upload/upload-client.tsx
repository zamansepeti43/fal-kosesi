...
import { ArrowLeft, Camera, ChevronRight, Coffee, ImagePlus, Sparkles, Upload as UploadIcon, X } from "lucide-react";
...
export default function UploadClient() {
  ...
  const activeCommentator = commentator ?? fallback ?? null;
  const price = activeCommentator?.priceCredits ?? 10;
  ...
  const next = () => {
    ...
    if (step === 1 && files.length === 0) return setError("Fal için en az 1 fincan fotoğrafı yükle.");
  ...
  const startReading = async () => {
    if (!activeCommentator || files.length === 0) return;
    setLoading(true);
    setError("");
    try {
      ...
      sessionStorage.setItem("falCommentatorId", activeCommentator.id);
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <section className="overflow-hidden rounded-[30px] border border-white/10 bg-[#0d0b18]/95 shadow-2xl">
          <div className="relative overflow-hidden px-5 pb-6 pt-8 text-center sm:px-8">
            ...
            <p className="relative mx-auto mt-2 max-w-lg text-xs leading-6 text-slate-400">Falcını seç, fincan fotoğraflarını gönder. Gerisini falcın ve Fal Köşesi senin için hazırlasın.</p>
          </div>

          <div className="border-t border-white/5 px-5 py-5 sm:px-8">
            <div className="mb-6 flex items-center justify-center gap-2">{[0,1,2].map((item) => <span key={item} className={`h-2 w-10 rounded-full transition ${item <= step ? "bg-rose-400" : "bg-white/10"}`}/>)}</div>

            {step === 0 && <div className="space-y-4">
              <div className="rounded-2xl border border-rose-300/15 bg-rose-300/[.04] p-4"><p className="font-serif text-xl font-bold">Falcını seç ✨</p><p className="mt-1 text-xs leading-5 text-slate-400">Her karakter farklı yorum tonu ve uzmanlıkla hazırlanmış sanal bir falcıdır. Soru formu yok; doğrudan fincanına geçebilirsin.</p></div>
            ...
            {step === 2 && <div className="space-y-4">
              <div className="text-center"><p className="text-[9px] font-bold uppercase tracking-[.25em] text-amber-200">Son kontrol</p><h2 className="mt-1 font-serif text-2xl font-bold">Falını başlatalım mı?</h2><p className="mt-1 text-xs text-slate-500">Kredi yalnızca aşağıdaki butona bastığında kullanılır. Soru formu yok; falın doğrudan fincanından okunacak.</p></div>
              <div className="rounded-[24px] border border-rose-300/15 bg-rose-300/[.04] p-4"><div className="flex items-center gap-3"><img src={`/api/fal/avatar?id=${encodeURIComponent(activeCommentator?.id ?? "coffee-esmeralya")}`} alt="Seçilen sanal karakter" className="h-16 w-16 rounded-2xl bg-[#21152d] object-cover"/><div><p className="font-serif text-lg font-bold">{activeCommentator?.name}</p><p className="text-xs text-violet-200">{activeCommentator?.title}</p><p className="mt-1 text-[10px] text-slate-500">AI karakter • gerçek kişi değildir</p></div><strong className="ml-auto text-lg text-amber-200">{price} kredi</strong></div><div className="mt-4 space-y-2 border-t border-white/5 pt-3 text-xs text-slate-400"><p>✓ {files.length} fincan fotoğrafı hazır</p><p>✓ {activeCommentator?.name} yorum tarzı seçildi</p><p>✓ Fal hazırlandığında bildirim oluşturulacak</p></div></div>
              {error && <p className="rounded-xl border border-red-400/20 bg-red-400/5 p-3 text-xs text-red-200">{error}</p>}
